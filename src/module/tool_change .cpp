...
inline void fast_line_to_current(const AxisEnum fr_axis) { _line_to_current(fr_axis); }
  ...
  void rp_tool_change(const uint8_t new_tool, bool no_move) 
  {
    #if EXTRUDERS == 0
      ...
      const bool can_move_away = !no_move && !idex_full_control;
        ...
        const bool should_swap = can_move_away && toolchange_settings.swap_length;
          ...
          const bool too_cold = !DEBUGGING(DRYRUN) && (thermalManager.targetTooColdToExtrude(old_tool));
        #else
      ...
      if (new_tool != old_tool) 
      {
        ...
        if (safe_to_move && !no_move && IsRunning()) 
        {
            ...
            if (singlenozzle_temp[new_tool] != singlenozzle_temp[old_tool]) {
              ...
              #endif
              (void)thermalManager.wait_for_hotend(new_tool, false);  // Wait for heating or cooling
            }
            */
